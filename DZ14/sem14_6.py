...
import json
import os
from exception import LevelError, AccessError
import pytest
...
class Access:
    ...
    def _add_user(self) -> list:
        ...
        if level_user == '1':
            ...
            raise LevelError(
                f"Пользователь {name_user} имеет {level_user} уровень. Для добавления новых пользователей уровень должнен быть =1")
...
def test_clear_str(test_start):
    assert test_start.users == [], 'возврат массива данных'

def test_input_data(test_start):
    assert test_start.check_access(["Denis",12]) == 'Пользователь есть в системе', 'проверка наличия пользователя'


if __name__ == "__main__":
    pytest.main(["-v"])
    # У меня, похоже очень запутанная система и я не понял, как в этом варианте вызывать тесты